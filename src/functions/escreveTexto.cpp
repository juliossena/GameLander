...
void escreverTexto(Rgb rgb, void * font, char *s, Position *position) {
    ...
    glRasterPos3f(position->getX(), position->getY(), 0);
...
void escreveHUD(Rgb rgb, Rgb alternative, float velocidade, float orientacaoEmGraus, float y, int combustivel, float pontuacao, int vidas) {
	...
	escreverTexto(rgb, GLUT_BITMAP_TIMES_ROMAN_10, str, new Position(-90, 90));

	if (orientacaoEmGraus - 90 == 0) {
		escreverTexto(rgb, GLUT_BITMAP_TIMES_ROMAN_10, str2, new Position(-90, 85));
	} else {
		escreverTexto(alternative, GLUT_BITMAP_TIMES_ROMAN_10, str2, new Position(-90, 85));
	}

	escreverTexto(rgb, GLUT_BITMAP_TIMES_ROMAN_10, str3, new Position(-90, 80));
	if (combustivel > 100) {
		escreverTexto(rgb, GLUT_BITMAP_TIMES_ROMAN_10, str4, new Position(-90, 75));
	} else {
		escreverTexto(alternative, GLUT_BITMAP_TIMES_ROMAN_10, str4, new Position(-90, 75));
	}

	escreverTexto(rgb, GLUT_BITMAP_TIMES_ROMAN_10, str5, new Position(-90, 70));
	escreverTexto(rgb, GLUT_BITMAP_TIMES_ROMAN_10, str6, new Position(-90, 65));
...
bool gerarPossuirBonus(float chance) {
	int numberChance = 1.0 / chance;
	int number = numberRandon(1, numberChance);
